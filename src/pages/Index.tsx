...
const Index = () => {
  ...
  const isBreakingCategory = (category?: { name?: string; slug?: string }) => {
    const name = category?.name?.toLowerCase() || "";
    const slug = category?.slug?.toLowerCase() || "";
    return name.includes("breaking") || slug.includes("breaking");
  };

  const breakingNewsArticles = safeArticles.filter((article) => {
    if (isBreakingCategory({ name: article.categoryName, slug: article.categorySlug })) return true;
    return article.categories?.some((cat) => isBreakingCategory(cat));
  });

  const featuredArticle = breakingNewsArticles[0] || null;
  const latestArticles = safeArticles.filter((article) => article.id !== featuredArticle?.id).slice(0, 3);
  const firstBreakingNews = breakingNewsArticles[0];
  const categorySections = categories.filter((cat) => !isBreakingCategory(cat)).slice(0, 3);
      ...
      <main className="flex-1">
        ...
        <section className="container mt-4">
          {featuredArticle ? (
            ...
            <div className="border border-dashed border-primary/40 rounded-sm p-6 md:p-10 text-center">
              <p className="font-body text-sm text-muted-foreground">
                No breaking news articles available yet.
              </p>
            </div>
          )}
        </section>
          ...
          <section className="mt-6 overflow-hidden">
            <div className="container px-0 sm:px-4">
              <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 px-4 sm:px-0 pb-4 scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                ...
                {breakingNewsArticles.slice(0, 4).map((article) => (
                  <div
                    key={article.id}
